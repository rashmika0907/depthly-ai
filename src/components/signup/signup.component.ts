    ...
    <div class="w-full max-w-md">
      <div class="glass-card p-8 shadow-2xl">
        <div class="text-center mb-8">
          <h1 class="text-3xl font-bold text-white">${translate('signup')}</h1>
          <p class="text-stone-400 mt-2">Join Depthly and start exploring</p>
        </div>
        <form onSubmit=${handleSubmit} novalidate>
          <div class="mb-4">
            <label for="name" class="block text-stone-300 mb-2">${translate('name')}</label>
          ...
          <button type="submit" disabled=${isLoading}
            ...
            : html`<span>${translate('signup')}</span>`}
          </button>
          <p class="text-center text-stone-400 mt-6">
            ${translate('signupPrompt')} <a href="#/login" class="font-medium text-rose-500 hover:underline">${translate('login')}</a>
          </p>
        </form>
      </div>
    </div>
